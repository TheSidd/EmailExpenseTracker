...

{
    ...
    var threads = GmailApp.search("newer_than:3d AND in:inbox AND from:axisbank.com AND subject:Transaction alert XX2232 OR XX9424 AND -label:axis_processed", 0, 100);
...

{
...

    {
        ...
        var matches = textWithoutBreaks.match(/([0-9]+)\s+for\s+([A-Za-z]+)\s+(.*)\s+at\s+(.*)\s+on\s*(\d{2}-\d+-\d+\s\d+:\d+:\d+)/);
...
function saveDataToSheet(records) {
    ...
    var spreadsheet = SpreadsheetApp.openByUrl("https://docs.google.com/spreadsheets/d/1vz8jHsEichGezMsORiFrsKTbENfKnomKJxQIkH9UtMI/edit#gid=1439879152");
